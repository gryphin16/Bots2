...
io.on('connection', function(socket) {
  ...
  socket.on('collectMass', function(data) {
    ...
    io.sockets.in(socket.room).emit('collectMass', data);
  });
socket.on('stopcollectMass', function(data) {
	  console.log("stopping collecting mass")
    io.sockets.in(socket.room).emit('stopcollectMass', data);
  });
