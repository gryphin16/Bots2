...
var trixnickname = config.useStaticName;
if(config.smartBots){
config.minimumMassBeforeFeed = config.neededMassToFeed;
  if(config.smartBotsLevel = 2){
    ...
    attachEvents: function() {
        ...
        bot.client.once('leaderBoardUpdate', function(old, leaders) {
            var name_array = leaders.map(function(ball_id) {
                return bot.client.balls != undefined ? bot.client.balls[ball_id].name : 'unnamed';
            });
            if (config.verbosityLevel > 0) {
...
socket.on('cmd', function(data) {
    ...
    } else if (data.name == "restart") {
setTimeout(function() {
    throw new Error('restart')
}, 20)
    } else if (data.name == "tricksplit") {
setTimeout(function() {
    for (bot in bots) {
        bots[bot].client.split();
    }
...
socket.on('collectMass', function(data) {
	console.log("collecting mass")
    config.botMode = config.botMode == 'blind' ? 'default' : 'blind';
});
    socket.emit("login", {
